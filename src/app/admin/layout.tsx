...
export default function AdminLayout({
  ...
  if (!isHydrated) {
    return <div className="min-h-screen bg-[#052e16]" />;
  }

  if (!isLoggedIn) {
    return (
      <div className={`min-h-screen flex items-center justify-center ${isDark ? 'bg-[#052e16]' : 'bg-[#f0fdf4]'}`}>
        <div className={`w-full max-w-md p-8 rounded-3xl ${isDark ? 'bg-gradient-to-br from-green-900/80 to-[#022c17] border border-green-700/30' : 'bg-white border border-green-200 shadow-xl'}`}>
          <div className="text-center mb-8">
            ...
            <h1 className={`text-2xl font-bold mb-2 ${isDark ? 'text-green-100' : 'text-green-900'}`}>
              Admin Login
            </h1>
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            <div>
              <label className={`block text-xs font-semibold mb-2 tracking-wider uppercase ${isDark ? 'text-green-400' : 'text-green-700'}`}>
                Password Admin
              </label>
              <input
                type="password"
  ...
  return (
    <div className={`min-h-screen flex ${isDark ? 'bg-[#052e16]' : 'bg-gray-50'}`}>
      {/* Sidebar */}
      <aside
        className={`fixed inset-y-0 left-0 z-50 w-64 transform transition-transform duration-300 ease-in-out ${
          sidebarOpen ? 'translate-x-0' : '-translate-x-full'
        } lg:translate-x-0 ${
          isDark ? 'bg-gradient-to-b from-green-900/90 to-[#022c17] border-r border-green-800' : 'bg-white border-r border-green-200'
        }`}
      >
        <div className="flex flex-col h-full">
          ...
          <div className="p-6 border-b border-green-700/20">
            <Link href="/admin" className="flex items-center gap-3">
              ...
              <div>
                <span className={`font-bold text-sm ${isDark ? 'text-green-100' : 'text-green-900'}`}>SEJUTA POHON</span>
                <span className={`block text-[10px] tracking-widest uppercase ${isDark ? 'text-green-600' : 'text-green-500'}`}>Admin Panel</span>
              </div>
            </Link>
          </div>
      ...
      <div className="flex-1 lg:ml-64">
        {/* Top Bar */}
        <header className={`sticky top-0 z-30 border-b ${isDark ? 'bg-[#052e16]/80 backdrop-blur-sm border-green-800' : 'bg-white/80 backdrop-blur-sm border-green-200'}`}>
